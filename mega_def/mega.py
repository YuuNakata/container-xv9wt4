...
class Mega:
    ...
    def _login_user(self, email, password):
        ...
        try:
            ...
            # v2 user account
            pbkdf2_key = hashlib.pbkdf2_hmac(hash_name="sha512", password=password.encode(), salt=a32_to_str(user_salt), iterations=100000, dklen=32)
        ...
        user = self._api_request(
            {
                ...
                "ts": base64_url_encode(a32_to_str(session_self_challenge) + a32_to_str(encrypt_key(session_self_challenge, master_key))),
            }
        )
        ...
        for file in list(files.items()):
            ...
            try:
                if parent_dir_name:
                    ...
                    if filename and parent_node_id and file[1]["a"] and file[1]["a"]["n"] == filename and parent_node_id == file[1]["p"]:
    ...
    def get_folder_link(self, file):
        ...
        if "h" in file and "k" in file:
            ...
            return f"{self.schema}://{self.domain}" f"/#F!{public_handle}!{decrypted_key}"
        else:
            raise ValidationError("File id and key must be present")
    ...
    def download(self, file, dest_path=None, dest_filename=None, progress_func=None):
        """
    ...
    def export(self, path=None, node_id=None):
        ...
        encrypted_node_key = base64_url_encode(share_key_cipher.encrypt(a32_to_str(node_key)))
    ...
    async def _download_file(
        self, file_handle, file_key, dest_path=None, dest_filename=None, is_public=False, file=None, progressfunc=None, args=None, f_data=None
    ):
        if file is None:
            if is_public:
        ...
        def request_get():
            return session.get(file_url, stream=True)

        # input_file =  requests.get(file_url, stream=True,timeout=5).raw
        input_file = await request_get()
        input_file = input_file.raw
    ...
    async def _download_file_from_folder(
        self, file_handle, file_key, dest_path=None, dest_filename=None, is_public=False, file=None, progressfunc=None, args=None, f_data=None
    ):
        if file is None:
    ...
    def upload(self, filename, dest=None, dest_filename=None):
        ...
        with open(filename, "rb") as input_file:
            ...
            data = self._api_request(
                {"a": "p", "t": dest, "i": self.request_id, "n": [{"h": completion_file_handle, "t": 0, "a": encrypt_attribs, "k": encrypted_key}]}
            )
    ...
    def _mkdir(self, name, parent_node_id):
        ...
        # update attributes
        data = self._api_request(
            {"a": "p", "t": parent_node_id, "n": [{"h": "xxxxxxxx", "t": 1, "a": encrypt_attribs, "k": encrypted_key}], "i": self.request_id}
        )
        return data
    ...
    def rename(self, file, new_name):
        ...
        # update attributes
        return self._api_request([{"a": "a", "attr": encrypt_attribs, "key": encrypted_key, "n": file["h"], "i": self.request_id}])
    ...
    def _edit_contact(self, email, add):
        ...
        if not re.match(r"[^@]+@[^@]+\.[^@]+", email):
            ...
            return self._api_request({"a": "ur", "u": email, "l": l, "i": self.request_id})
    ...
    def import_public_file(self, file_handle, file_key, dest_node=None, dest_name=None):
        ...
        return self._api_request({"a": "p", "t": dest_node["h"], "n": [{"ph": file_handle, "t": 0, "a": encrypted_name, "k": encrypted_key}]})
